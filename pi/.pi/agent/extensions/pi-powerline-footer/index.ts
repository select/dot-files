    ...
    import("@mariozechner/pi-coding-agent").then(({ CustomEditor }) => {
      ...
      const editorFactory = (tui: any, editorTheme: any, keybindings: any) => {
        ...
        editor.render = (width: number): string[] => {
          ...
          // Custom border color: muted dark blue-grey instead of theme's sep color
          const bc = (s: string) => `${ansi.getFgAnsi(80, 80, 100)}${s}${ansi.reset}`;
          
          // Content area: 2 chars for prompt prefix ("╰ " / "  ")
          const promptPrefix = `${ansi.getFgAnsi(80, 80, 100)}╰${ansi.reset} `;
          const contPrefix = "  ";
          ...
          // Top border with segments merged in: ╭─ [segments] ──────────────────
          // Saves 1 line vs separate status bar + plain top border
          const layout = getResponsiveLayout(width, ctx.ui.theme);
          const segContent = layout.topContent; // Has leading/trailing spaces
          const segVisible = visibleWidth(segContent);
          const cornerAndDash = bc("╭─");
          const cornerWidth = 2; // "╭─"
          const remainingWidth = Math.max(0, width - cornerWidth - segVisible);
          result.push(cornerAndDash + segContent + (remainingWidth > 0 ? bc("─".repeat(remainingWidth)) : ""));
          ...
          // No bottom border — the ╰ on the prompt line is the closing corner
