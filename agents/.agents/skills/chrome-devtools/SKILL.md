...
### Test
```bash
bun run navigate.js --url https://example.com
...
pwd  # Should show current working directory
cd .claude/skills/chrome-devtools/scripts
bun run screenshot.js --url https://example.com --output ./docs/screenshots/page.png
```
...
bun run screenshot.js --url https://example.com --output page.png

# Custom size threshold (e.g., 3MB)
bun run screenshot.js --url https://example.com --output page.png --max-size 3

# Disable compression
bun run screenshot.js --url https://example.com --output page.png --no-compress
```
...
bun run navigate.js --url https://example.com/login --close false
bun run fill.js --selector "#email" --value "user@example.com" --close false
bun run fill.js --selector "#password" --value "secret" --close false
bun run click.js --selector "button[type=submit]"
```

### Parse JSON Output
```bash
# Extract specific fields with jq
bun run performance.js --url https://example.com | jq '.vitals.LCP'

# Save to file
bun run network.js --url https://example.com --output /tmp/requests.json
```
...
```bash
# CSS selector fails
bun run click.js --url https://example.com --selector ".btn-submit"
# Error: waiting for selector ".btn-submit" failed

# Discover correct selector
bun run snapshot.js --url https://example.com | jq '.elements[] | select(.tagName=="BUTTON")'

# Try XPath
bun run click.js --url https://example.com --selector "//button[contains(text(),'Submit')]"
```
...
### Web Scraping
```bash
bun run evaluate.js --url https://example.com --script "
...
### Performance Testing
```bash
PERF=$(bun run performance.js --url https://example.com)
...
### Form Automation
```bash
bun run fill.js --url https://example.com --selector "#search" --value "query" --close false
bun run click.js --selector "button[type=submit]"
```

### Error Monitoring
```bash
bun run console.js --url https://example.com --types error,warn --duration 5000 | jq '.messageCount'
```
...
Use `snapshot.js` to discover selectors:
```bash
bun run snapshot.js --url https://example.com | jq '.elements[] | {tagName, text, selector}'
```
...
**Element not found**
- Get snapshot first to find correct selector: `bun run snapshot.js --url <url>`
